$(function(){
    ...
    $.ajax({
        ...
        success:function(data,kl){
            for(var i=1;i>=0;i--){        
                var that=$('<div></div>').addClass('col-sm-10 col-sm-offset-1').css({'paddingTop':'3%','paddingBottom':'5%' });
                $('<a href="" class="col-sm-3 img col-xs-4"><img src="'+'http://123.58.241.146:8088/'+data.data[i].smallImg+'" alt=""></a><div class="col-sm-8  col-xs-8"><h3>项目名称：'+data.data[i].title+'</h3><h3>'+data.data[i].subTitle+'</h3><p><span>项目简介:</span>'+data.data[i].intro+'</p></div>').appendTo(that);
